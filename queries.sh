...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games ON games.winner_id = teams.team_id OR opponent_id = teams.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
...
echo "$($PSQL "SELECT CONCAT(year, '|', name) FROM teams FULL JOIN games ON games.winner_id = teams.team_id WHERE round = 'Final' ORDER BY year")"
